...
BOT_TOKEN = "ВАШ_ТОКЕН"
CHANNEL = "@ВАШ_КАНАЛ"  # например @myquizbot_channel
...
TEXTS = {
    'ru': {
        ...
        'not_subscribed': f"❌ Для использования бота подпишись на канал {CHANNEL}\n\nПосле подписки нажми /start",
        'menu': (
            "📋 Меню:\n\n"
    ...
    'en': {
        ...
        'not_subscribed': f"❌ To use the bot, subscribe to {CHANNEL}\n\nAfter subscribing press /start",
        'menu': (
            "📋 Menu:\n\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("play_"):
        ...
        if next_index < len(quiz['questions']):
            ...
            await query.message.reply_text(
                t(user_id, 'finished', score=score, total=total, code=code)
            )
